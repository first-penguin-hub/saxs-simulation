...
def init_state() -> None:
    defaults = {
        ...
        # Radial Profile (shell-like, target Rg)
        "R_nm_rad": 300.0,
        "Rm_nm_rad": 10.0,
        "n_rad": 500,
        "Rg_target_rad": 260.0,
...
st.title("Fractal cluster analysis app")
...
with st.sidebar:
    ...
    mode = st.radio(
        ...
        index=0 if st.session_state.mode == "Fractal SAXS" else 1,
    )
    ...
    if mode == "Fractal SAXS":
        ...
        st.header("Radial Profile parameters")
        with st.form("radial_form"):
            R_nm = st.number_input("Container radius R [nm]", min_value=1.0, value=float(st.session_state.R_nm_rad), step=10.0)
            Rm_nm = st.number_input("Particle radius Rm [nm]", min_value=0.05, value=float(st.session_state.Rm_nm_rad), step=0.5)
            n_particles = st.number_input("Number of particles n", min_value=1, value=int(st.session_state.n_rad), step=10)
            Rg_target = st.number_input("Target Rg [nm]", min_value=0.1, value=float(st.session_state.Rg_target_rad), step=1.0)
            ...
            n_bins = st.number_input("Radial profile bins", min_value=5, value=int(st.session_state.n_bins_rad), step=1)
            ...
            q_min = st.number_input("q_min [nm^-1]", min_value=0.00001, value=float(st.session_state.q_min_rad), step=0.00010, format="%.5f")
            q_max = st.number_input("q_max [nm^-1]", min_value=0.00010, value=float(st.session_state.q_max_rad), step=0.01000, format="%.5f")
            n_q = st.number_input("Number of q points", min_value=10, value=int(st.session_state.n_q_rad), step=10)
        ...
        if run:
            ...
            st.session_state.q_min_rad = float(q_min)
            st.session_state.q_max_rad = float(q_max)
            st.session_state.n_q_rad = int(n_q)
...
if mode == "Fractal SAXS":
    ...
    if st.session_state.get("run_radial", False):
        try:
            with st.spinner("Generating shell-like configuration, computing radial profile and scattering..."):
            ...
            c3.metric("Guinier Rg [nm]", f"{info['Rg_guinier_nm']:.3f}")
            c4.metric("Effective Df", f"{info['Df_effective_radial']:.3f}")
            c5.metric("Shell bias α", f"{info['alpha']:.3f}")
            ...
            d3.download_button("I(q) CSV", iq_csv.getvalue(), file_name="shell_like_Iq.csv", mime="text/csv", use_container_width=True)
            d4.download_button("Summary TXT", summary_txt.getvalue(), file_name="radial_profile_summary.txt", mime="text/plain", use_container_width=True)
        except Exception as e:
            st.error(str(e))
    else:
