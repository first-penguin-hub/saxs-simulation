...
def sample_fractal_radii(n: int, R_nm: float, Df: float, rng: np.random.Generator) -> np.ndarray:
    u = rng.random(n)
    return R_nm * np.power(u, 1.0 / Df)
...
def estimate_max_particles(R_nm: float, d_nm: float, packing_fraction: float = 0.35) -> float:
...
def radial_concentration_profile(pos: np.ndarray, R_nm: float, n_bins: int = 30) -> Dict[str, np.ndarray]:
    if n_bins < 2:
    ...
    cumulative_fraction = np.cumsum(counts) / len(r)
